#ifndef MOONY_QUEUE_H_
#define MOONY_QUEUE_H_
...
#define STAILQ_REMOVE_AFTER(head, elm, field) do {				\
	if ((STAILQ_NEXT(elm, field) =								\
	     STAILQ_NEXT(STAILQ_NEXT(elm, field), field)) == nullptr)	\
		(head)->stqh_last = &STAILQ_NEXT((elm), field);			\
} while (0)
...
#define LIST_SWAP(head1, head2, type, field) do {		\
	...
	if ((swap_tmp = LIST_FIRST((head1))) != nullptr)		\
		swap_tmp->field.le_prev = &LIST_FIRST((head1));	\
	if ((swap_tmp = LIST_FIRST((head2))) != nullptr)		\
...
#define	QMD_TAILQ_CHECK_TAIL(head, field) do {					\
	if (*(head)->tqh_last != nullptr)								\
	    	panic("Bad tailq NEXT(%p->tqh_last) != nullptr", (head)); 	\
} while (0)
