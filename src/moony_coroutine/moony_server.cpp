...
#define SERVER_STRING "Server: moony_server\r\n"
#define STDIN   0
#define STDOUT  1
#define STDERR  2

#define ENABLE_MOONY	1
#define BASE_PORT			9096
#define PORT_CNT 			20
...
void accept_request(void *arg)
{
    ...
    sprintf(path, "/home/yusuke%s", url);
...
void server(void *arg) {
    ...
    while (1)
    {
        ...
        *client_sock = accept(server_sock,
		...
		moony_coroutine *read_co;
		moony_coroutine_create(&read_co, accept_request, client_sock);
...
int main(int argc, char* argv[]) {
	moony_coroutine* co = NULL;

	for (int i = 0; i < PORT_CNT; i ++) {
		unsigned short* port = (unsigned short*)calloc(1, sizeof(unsigned short));
		*port = BASE_PORT + i;
		moony_coroutine_create(&co, server, port); ////////no run
	}
	
	moony_schedule_run(); //run
